...
- **Email:** Resend SDK (`resend` package) for transactional emails — welcome, registration OTP, login OTP, login alerts, forgot password OTP, withdrawal OTP, deposit/withdrawal status updates, KYC status notifications, support ticket + confirmation. Uses `RESEND_API_KEY` secret. Sandbox FROM: `TradeX AI <onboarding@resend.dev>`. Email service in `server/emailService.ts`. OTP codes stored in `email_otp_codes` table (hashed, with expiry + purpose). Registration flow: user registers (email, password, country, phone) → account created with `isEmailVerified: false` → OTP sent → user verifies → session created + welcome email sent. Login flow: credentials verified → OTP sent → user verifies → session created + login alert sent. Forgot password flow: `/forgot-password` page (3-step: email → OTP → new password). Withdrawal OTP flow: Wallet.tsx sends OTP → verifies → gets token → submits withdrawal with token. Resend OTP endpoint: `/api/auth/resend-otp` (supports `registration` and `login` purposes).
- **Support Page:** `/support` route accessible to both authenticated and unauthenticated users. Department select (Payment & Billing, KYC & Verification, Trading & Signals, Wallet & Deposits, Withdrawals, Account & Security, Technical Issues, General Inquiry), subject, description. Sends ticket email to `support@tradexai.in` + confirmation email to user. Rate-limited to 3 requests per 15 minutes per IP. Server-side email validation.
- **Registration Fields:** Country dropdown (60+ countries with dial codes), optional mobile number with auto-populated country code prefix. Data sent to backend: `country`, `countryCode`, `phone`.
- **Company Address:** Office 1205, Jumeirah Bay X2 Tower, Cluster X, JLT, Dubai, UAE. Shown in Landing.tsx footer and Support.tsx page.