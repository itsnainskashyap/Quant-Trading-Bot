...
- **Deposit/Withdraw System:** Comprehensive wallet system supporting crypto deposits (BTC, ETH, USDT, LTC, USDC across multiple chains) and UPI deposits with INR→USDT conversion (92 INR = 1 USDT). Admin-managed payment methods, deposit/withdrawal approval workflow with status transitions (pending→processing→approved/rejected), idempotent state machine for balance integrity, transactional withdrawal deductions. Admin wallet routes require `x-admin-session` header auth. Frontend: `/wallet` route with Deposit/Withdraw/History tabs. Dashboard header shows USDT wallet balance with link to wallet page. Visual crypto/chain logo selectors using cryptologos.cc CDN images. UPI logo from attached asset.
- **KYC Verification System:** AI-powered identity verification using OpenAI GPT-4o Vision. Supports Aadhaar Card, PAN Card, Voter ID, and Government ID. AI extracts name, DOB, document number, gender, address from uploaded documents. Auto-verification for high-confidence (85%+) valid documents. Manual admin review for lower confidence submissions. KYC is mandatory before making deposits. Admin KYC management section in Admin panel. KYC status shown on Profile page and Wallet page with appropriate badges. Routes: `/api/kyc/status`, `/api/kyc/submit`, `/api/admin/kyc`, `/api/admin/kyc/:id/action`. Frontend: `/kyc` route. Schema: `kyc_documents` table in trading.ts.
